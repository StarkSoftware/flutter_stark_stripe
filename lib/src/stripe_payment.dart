...
class StripePayment {
  ...
  static Future<Token> paymentRequestWithNativePay(
      { AndroidPayPaymentRequest androidPayOptions,  ApplePayPaymentOptions applePayOptions}) {
    if (kIsWeb) {
      throw UnimplementedError();
    } else {
      if (Platform.isAndroid) {
  ...
  static Future<PaymentIntentResult> authenticatePaymentIntent({ String clientSecret}) async {
  ...
  static Future<SetupIntentResult> authenticateSetupIntent({ String clientSecret}) async {
    assert(clientSecret != null);
...
class StripeOptions {
  ...
  StripeOptions({ this.publishableKey, this.merchantId, this.androidPayMode});
